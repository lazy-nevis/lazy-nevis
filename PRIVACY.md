...
The application has no telemetry, advertising, accounts, cloud synchronization, or automatic updater. The About page offers an explicit manual update check that requests only public release metadata from `api.github.com/repos/lazy-nevis/lazy-nevis`. It sends no settings, session, window, or activity data. GitHub may receive ordinary network metadata such as the user's IP address. Results are cached for 15 minutes, and the app never downloads or executes an update.
